...
// ─── Photo Scan Panel ─────────────────────────────────────────────────────
function PhotoScanPanel({ onSelect }) {
        ...
        body: JSON.stringify({
          ...
          messages: [{
            ...
            content: [
              ...
              {
                type: "text",
                text: `Look at this book cover image and extract the book details. 
Reply with ONLY a JSON object, no other text, no markdown fences:
{
  ...
  "genre": "one of: Psychological Thriller, Thriller, Mystery, Cozy Mystery, Dystopian Fiction, Science Fiction, Fantasy, Historical Fiction, Contemporary Fiction, Contemporary Romance, Historical Romance, Erotic Romance, New Adult Romance, Literary Fiction, Women's Fiction, Suspense, Horror, Other",
  "confidence": "high|medium|low"
}
If you cannot read the cover clearly, set confidence to "low" and do your best.`
              }
            ]
          }]
        })
      ...
      setErrorMsg("Could not read the book cover. Try a clearer photo.");
      setPhase("error");
    }
  };
  ...
  return (
    <div>
      ...
      {phase === "idle" && (
        <div>
          <div style={{ background: "#f5f0ff", borderRadius: 12, padding: "12px 14px", marginBottom: 14, fontSize: 13, color: "#3a1a6e", lineHeight: 1.6 }}>
            <div style={{ fontWeight: 700, marginBottom: 4 }}>📷 How it works:</div>
            Take or upload a photo of any book cover — Claude AI will read the title, author and genre automatically.
          </div>
          <button onClick={() => fileRef.current.click()}
            style={{ width: "100%", padding: "28px 0", borderRadius: 16, border: "2px dashed #c4b0f0", background: "#faf8ff", color: "#4a1a8a", fontSize: 15, fontWeight: 700, cursor: "pointer", textAlign: "center", display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
            <span style={{ fontSize: 44 }}>📷</span>
            <span>Take Photo or Choose Image</span>
            <span style={{ fontSize: 12, color: "#aaa", fontWeight: 400 }}>JPG, PNG — any book cover</span>
          </button>
        </div>
      )}
        ...
        <div>
          <div style={{ display: "flex", justifyContent: "center", marginBottom: 14 }}>
            <img src={imgSrc} alt="Book cover"
              style={{ maxHeight: 220, maxWidth: "100%", borderRadius: 12, boxShadow: "0 4px 16px rgba(0,0,0,0.15)", objectFit: "contain" }} />
          </div>
          {errorMsg && (
            <div style={{ background: "#fff5f5", border: "1.5px solid #ffcccc", borderRadius: 10, padding: "10px 14px", marginBottom: 12, fontSize: 13, color: "#c62828", textAlign: "center" }}>
              {errorMsg}
            </div>
          )}
          <div style={{ display: "flex", gap: 9 }}>
            <button onClick={handleRetake}
              style={{ padding: "13px 0", borderRadius: 12, border: "1.5px solid #e0d8f8", background: "#fff", color: "#666", fontSize: 14, fontWeight: 700, cursor: "pointer", width: 80, flexShrink: 0 }}>
              Retake
            </button>
            <button onClick={handleScan}
              style={{ flex: 1, padding: "13px 0", borderRadius: 12, border: "none", background: "linear-gradient(135deg,#4a1a8a,#7c3aed)", color: "#fff", fontSize: 15, fontWeight: 700, cursor: "pointer", boxShadow: "0 4px 14px rgba(74,26,138,0.25)" }}>
              🔍 Scan Book Cover
            </button>
          </div>
        </div>
      ...
      {phase === "scanning" && (
        <div style={{ textAlign: "center", padding: "40px 0" }}>
          ...
          <div style={{ marginTop: 16, fontFamily: "'Playfair Display', serif", fontSize: 16, color: "#4a1a8a", fontWeight: 700 }}>Reading the cover…</div>
          <div style={{ marginTop: 6, fontSize: 13, color: "#aaa" }}>Claude AI is identifying the book</div>
        </div>
      )}
      ...
      {phase === "done" && result && (
        <div>
          <div style={{ display: "flex", gap: 14, marginBottom: 16, alignItems: "flex-start" }}>
            <img src={imgSrc} alt="" style={{ width: 60, height: 86, objectFit: "cover", borderRadius: 8, flexShrink: 0, boxShadow: "0 2px 8px rgba(0,0,0,0.15)" }} />
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 10, color: result.confidence === "high" ? "#2d7a3a" : result.confidence === "medium" ? "#b85c00" : "#c62828", fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", marginBottom: 4 }}>
                {result.confidence === "high" ? "✓ High confidence" : result.confidence === "medium" ? "⚠ Medium confidence" : "⚠ Low confidence — please verify"}
              </div>
              <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 16, fontWeight: 800, color: "#1a0a2e", lineHeight: 1.3 }}>{result.title}</div>
              <div style={{ fontSize: 13, color: "#666", marginTop: 3 }}>by {result.author}</div>
              <div style={{ marginTop: 6 }}>
                <span style={{ fontSize: 11, fontWeight: 700, color: genreColors[result.genre] || "#888", background: (genreColors[result.genre]||"#888")+"18", padding: "2px 8px", borderRadius: 8 }}>{result.genre}</span>
              </div>
            </div>
          </div>
          <div style={{ display: "flex", gap: 9 }}>
            <button onClick={handleRetake}
              style={{ padding: "13px 0", borderRadius: 12, border: "1.5px solid #e0d8f8", background: "#fff", color: "#666", fontSize: 13, fontWeight: 700, cursor: "pointer", width: 80, flexShrink: 0 }}>
              Retry
            </button>
            <button onClick={() => onSelect({ ...result, cover: imgSrc })}
              style={{ flex: 1, padding: "13px 0", borderRadius: 12, border: "none", background: "linear-gradient(135deg,#2d7a3a,#43a854)", color: "#fff", fontSize: 15, fontWeight: 700, cursor: "pointer", boxShadow: "0 4px 14px rgba(45,122,58,0.3)" }}>
              ✓ Use This Book
            </button>
          </div>
        </div>
      ...
      <div onClick={e => e.stopPropagation()} style={{ background: "#fff", borderRadius: "24px 24px 0 0", width: "100%", maxWidth: 520, maxHeight: "92vh", overflowY: "auto", padding: "24px 20px 44px", boxShadow: "0 -8px 40px rgba(0,0,0,0.18)" }}>
        ...
        {isNew && (
          <div style={{ display: "flex", gap: 0, marginBottom: 16, borderRadius: 12, overflow: "hidden", border: "1.5px solid #ede8ff" }}>
            {[["search", "🔍 Search"], ["photo", "📷 Photo"], ["manual", "✏️ Manual"]].map(([key, label]) => (
              <button key={key} onClick={() => setTab(key)} style={{ flex: 1, padding: "10px 0", border: "none", background: tab === key ? "#4a1a8a" : "#fff", color: tab === key ? "#fff" : "#666", fontSize: 11, fontWeight: 700, cursor: "pointer", transition: "all 0.15s" }}>
                {label}
              </button>
            ))}
          </div>
          ...
          <>
            ...
            {(form.title) && (
              <div style={{ marginTop: 12, padding: "10px 14px", background: "#f5f0ff", borderRadius: 10, fontSize: 13, color: "#4a1a8a", fontWeight: 600 }}>
                ✓ Selected: <em>{form.title}</em> — tap "Manual" to edit details
              </div>
            )}
          </>
        )}
        ...
        {/* Photo Tab */}
        {tab === "photo" && isNew && (
          <PhotoScanPanel onSelect={(result) => {
            setForm(f => ({
              ...f,
